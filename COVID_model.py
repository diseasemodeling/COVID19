...
def run_COVID_sim(decision, path, beta_u, write = 'N'):
...
if  __name__ == "__main__":
    ...
    policy_range = [[0,a_sd_range],[0,1],[0,1]]
    init_policy = [policy_range[0][1], 0, 0]
    data_path = os.path.join(path, 'data/policy.xlsx')
    gv.decision = rp.read_policy(gv.T_max, policy_range, init_policy, data_path)
    # gv.decision = gv.format_year(gv.decision_week, gv.T_max)
   
    beta_u = gv.beta_after_sd    # assume keep social distancing  
   
    run_COVID_sim(path = path, beta_u = beta_u, decision = gv.decision)
